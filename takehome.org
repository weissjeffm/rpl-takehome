...
(defmacro threaded-if
  "Expands an expression that expects state inserted as the first arg
   to a let/if that returns the new state"
  [state if-clause else-clause]
  `(let [tos# (-> ~state :stack first)
         newstate# (update ~state :stack pop)]
     (if tos#
       (-> newstate# ~@if-clause)
...
(defn assign-initial-vars
  ...
  (let [;; discard the leading ! from var names
        vars (map (comp first get-var-key) vars)
